...
1. **In‑App Settings:** `Settings > Equibop Settings > Arguments > Configure`.
2. **Desktop entry:** Edit the `Exec` line in `/usr/share/applications/equibop.desktop` (or your local equivalent).
3. **Config file:** Add flags to `${XDG_CONFIG_HOME}/equibop-flags.conf` (see “The Flags File” section).
